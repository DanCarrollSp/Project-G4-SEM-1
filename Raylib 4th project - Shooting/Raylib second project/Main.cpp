...
float bobbingTime = 0.0f;    // Tracks time for the sine wave
float bobbingSpeed = 18.0f;   // Speed of  bobbing
...
int main(void)
{
    ...
    //Gun
    Texture2D handTexture = LoadTexture("resources/Hand.png");
    ...
    while (!WindowShouldClose())
    {
        ...
        if (isMoving) {
            ...
            bobbingOffset = sin(bobbingTime) * bobbingAmplitude;
        }
        else {
            bobbingTime = 0.0f;
        }
        ...
        float scale = 1.0f;
        int handPosX = (screenWidth - handTexture.width * scale) / 2;
        int handPosY = screenHeight - handTexture.height * scale + 10 + bobbingOffset;
        Vector2 handPos = {handPosX, handPosY};
        DrawTextureEx(handTexture, handPos, 0.0f, scale, WHITE);
